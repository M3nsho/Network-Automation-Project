...
def get_local_ip():
    try:
        ...
        s.connect(("8.8.8.8", 80))  # pretend connect — no data sent
        ip = s.getsockname()[0]
        s.close()
        return ip
    except Exception:
...
def sniff_packets():
    try:
        if os.name == 'nt':
            host = get_local_ip()
            print(f"[*] Detected local IP: {host}")
        ...
        while True:
            ...
            if ttl < TTL_THRESHOLD:
                ...
                last_seen = alert_cooldown.get(src_ip)
                if last_seen and (now - last_seen).seconds < 10:
                    continue

                alert_cooldown[src_ip] = now
